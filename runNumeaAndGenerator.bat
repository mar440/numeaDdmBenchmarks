...
REM ####     3 - '''' nothing '''
REM ####     4 - two squers (two pieces) in mutual contact - quasi-static problem
REM ####     5 - one beam (one piece) - time-harmonic problem
...
set SOLVERS=1
REM ####    -1  - only *.vtu and *.nma files are generated 
...
IF %SOLVERS% EQU -1 (
%PYTHON_EXE% %meshGeneratorPath%\%modifNumeaFilePy% NMA_FILE %NMA_FILE%  LinearSolver "DIRECT_DDSOLV" blablafile %blablafile% dumpfile %dumpfile% FETI %FETI% eps_iter %eps_iter% dumpfileCSRmatrix %dumpfileCSRmatrix%
...
@echo %mpi_size%
set USE_PARDISO=0
IF %SOLVERS% EQU 1 (set /A USE_PARDISO=%USE_PARDISO1% + 1)
IF %SOLVERS% EQU 3 (set /A USE_PARDISO=%USE_PARDISO1% + 1)
IF %SOLVERS% EQU 4 (set /A USE_PARDISO=%USE_PARDISO1% + 1)
...
IF %USE_PARDISO% GTR 0 ( 
%PYTHON_EXE% %meshGeneratorPath%\%modifNumeaFilePy% NMA_FILE %NMA_FILE%  LinearSolver "DIRECT_PARDISO" blablafile %blablafile% dumpfile %dumpfile% FETI %FETI% eps_iter %eps_iter% dumpfileCSRmatrix %dumpfileCSRmatrix%
